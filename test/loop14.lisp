...
(deftest loop.14.1
  (khazern:loop for x from 1 to 6
        when (evenp x)
        collect x)
  (2 4 6))
...
(deftest loop.14.13
  (khazern:loop for i from 1 to 20
        if (evenp i)
          if (= (mod i 3) 0)
...
(deftest loop.14.24
  (khazern:loop for x in '((a) nil (b) (c) (nil) (d))
...
(deftest loop.14.26
  (khazern:loop for x in '(nil (a) nil (b) (c) (nil) (d))
...
(deftest loop.14.31
  (khazern:loop for it on '(a b c d)
        when (car it) collect it)
  (a b c d))

(deftest loop.14.32
  (khazern:loop for x in '(a b nil c d nil e)
...
(deftest loop.14.38
  (khazern:loop for x in '(1 2 nil 3 4 nil 5 nil)
        when x count it)
  5)
...
(deftest loop.14.40
  (khazern:loop for x in '(1 2 nil 3 4 nil 6 nil)
        when x maximize it)
  6)
