...
(deftest loop.9.1
  (khazern:loop for x in '(2 3 4) collect (1+ x))
  (3 4 5))

(deftest loop.9.2
  (khazern:loop for x in '(2 3 4) collecting (1+ x))
  (3 4 5))

(deftest loop.9.3
  (khazern:loop for x in '(0 1 2)
        when (eql x 2) do (return 'good)
        collect x)
  good)

(deftest loop.9.4
  (khazern:loop for x in '(a b c)
...
(deftest loop.9.6
  (khazern:loop for x from 1 to 10
...
(deftest loop.9.7
  (khazern:loop for x from 1 to 10
        collect (if (> x 5) (khazern:loop-finish) x))
  (1 2 3 4 5))

(deftest loop.9.8
  (khazern:loop for x from 1 to 20
...
(deftest loop.9.12
  (let ((foo '(a b)))
    (values
     (khazern:loop for x in '(c d e) collect x into foo finally (return foo))
     foo))
  (c d e)
  (a b))
...
(deftest loop.9.20
  (khazern:loop for x in '((a b) (c d) (e f g) () (i)) append x)
  (a b c d e f g i))

(deftest loop.9.21
  (khazern:loop for x in '((a b) (c d) (e f g) () (i)) appending x)
  (a b c d e f g i))

(deftest loop.9.22
  (khazern:loop for x in '((a) (b) (c . whatever)) append x)
  (a b c . whatever))

(deftest loop.9.23
  (khazern:loop for x in '((a) (b) (c . whatever)) appending x)
  (a b c . whatever))

(deftest loop.9.24
  (khazern:loop for x in '(a b c d)
        append (list x)
...
(deftest loop.9.30
  (khazern:loop for x in '((a b) (c d) (e f g) () (i)) nconc (copy-seq x))
  (a b c d e f g i))

(deftest loop.9.31
  (khazern:loop for x in '((a b) (c d) (e f g) () (i)) nconcing (copy-seq x))
  (a b c d e f g i))

(deftest loop.9.32
  (khazern:loop for x in '((a) (b) (c . whatever)) nconc (cons (car x) (cdr x)))
  (a b c . whatever))

(deftest loop.9.33
  (khazern:loop for x in '((a) (b) (c . whatever)) nconcing (cons (car x) (cdr x)))
  (a b c . whatever))
...
(deftest loop.9.40
  (khazern:loop for x in '(1 2 3 4 5 6 7)
...
(deftest loop.9.42
  (khazern:loop for x in '(a b c d e) nconc (cons x 'foo))
  (a b c d e . foo))
...
(deftest loop.9.43
  (macrolet
   ((%m (z) z))
   (khazern:loop for x in '(1 2 3) collect (expand-in-current-env (%m (- x)))))
  (-1 -2 -3))

(deftest loop.9.44
  (macrolet
   ((%m (z) z))
   (khazern:loop for x in '(1 2 3) collecting (expand-in-current-env (%m (list x)))))
  ((1) (2) (3)))
...
(deftest loop.9.46
  (macrolet
   ((%m (z) z))
   (khazern:loop for x in '((a b) (c d) (e f g) () (i))
