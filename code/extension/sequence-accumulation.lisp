...
(defmethod khazern:scope-functions
  ...
  (let ((head (khazern:scope-reference instance :head))
    ...
    (khazern:with-gensyms (value args)
...
(defmethod khazern:scope-functions
  ...
  (let ((adjoin-name (khazern:scope-reference instance :adjoin)))
    (khazern:with-gensyms (value args item)
      (values `((,name (,value &rest ,args)
                  (map nil (lambda (,item)
...
(defmethod khazern:scope-functions
  ...
  (let ((head (khazern:scope-reference instance :head))
    ...
    (khazern:with-gensyms (value args next)
      (multiple-value-bind (definitions declarations)
...
(defmethod khazern:scope-functions
  ...
  (let ((head (khazern:scope-reference instance :head))
        (tail (khazern:scope-reference instance :tail)))
    (khazern:with-gensyms (value test key head item next)
      `((,name (,value &key ((:test ,test) #'eql) ((:key ,key) #'identity))
          (prog ((,head ,head))
           ,next
             (cond ((null (cdr ,head))
                    (return nil))
...
(defmethod khazern:scope-functions
  ...
  (let ((head (khazern:scope-reference instance :head))
        (tail (khazern:scope-reference instance :tail)))
    (khazern:with-gensyms (value test key head value-key next)
      `((,name (,value &key ((:test ,test) #'eql) ((:key ,key) #'identity))
          (prog ((,head ,head)
...
(defmethod khazern:scope-functions
  ...
  (let ((var (khazern:var-spec (khazern:var instance)))
        (type (khazern:type-spec (khazern:var instance))))
    (khazern:with-gensyms (value predicate key)
      (values `((,name (,value &key ((:predicate ,predicate) nil) ((:key ,key) nil))
